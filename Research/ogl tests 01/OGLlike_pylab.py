...
class ShapeCanvas(wx.ScrolledWindow):
    def __init__(self, parent, id=-1, pos=(-1, -1), size=(-1, -1), style=0, Design=None, name=""):
        ...
        self.Popup_Menu_Canvas = My_Popup_Menu(self._OnPopupItemSelected, 1, pre=pre, post=["ToDo"])
    ...
    # ************************************
    # ************************************
    def Set_Coordinates(self, LB=(-100, -100), WH=(200, None), RT=None):
        print(LB, WH, RT)
        if RT:
    ...
    # *********************************************************
    def W2Sx(self, x):
        return int(1.0 * self._W * (x - self.WCl) / (self.WCr - self.WCl))
    ...
    # *********************************************************
    def W2Sy(self, y):
        return int(1.0 * self._H - self._H * (y - self.WCb) / (self.WCt - self.WCb))
    ...
    def Print_All(self):
        ...
        for item in self.diagram.shapes:
            ...
            if isinstance(item, Rectangle):
                ...
                print("")
            print("    x, y, color  =", item._x, ",", item._y, ",", item.fill, ",", item.pen)

            for node in self.nodes:
                if node.item == item:
                    print(
                        "   ",
                        str(node.__class__).split(".")[1] + "  ---------------------------------",
                    )
                    print(
                        "      x, y, color  =", node._x, ",", node._y, ",", node.fill, ",", node.pen
                    )
            ...
            if self.Design:
                ...
                if self.RubberBand:
                    ...
                    self.RubberBand = t_RubberBand_Shape(
                        self.RubberBand_XY[0],
                        self.RubberBand_XY[1],
                        self.RubberBand_XY[0] + 10,
                        self.RubberBand_XY[1] + 10,
                    )
    ...
    def getEventCoordinates(self, event):
        ...
        return [
            round((event.GetX() + (originX * unitX)) / self.scalex),
    ...
    def select(self, item=None):
        ...
        if isinstance(item, Node) and not (
            (len(self.selectedShapes) == 1)
            and (isinstance(self.selectedShapes[0], Connection_Line))
        ):
            del self.selectedShapes[:]
        ...
        if not item in self.selectedShapes:
            ...
            if item.Resizeable:
                if item.Scene_2D:
                    self.nodes.extend(
                        [ResizeableNode_2D(self, item, n, self) for n in range(len(item._x))]
                    )
                else:
                ...
                if isinstance(item, Connection_Line):
                    ...
                    if not (item.output) and item.input:
                        self.Show_Inputs(item.input[0], item.input[0].Type_Outputs[item.input[1]])
...
class INode(Node):
    ...
    def move(self, x, y):
        ...
        self.Container_Canvas.Show_Outputs(self.item, self.item.Type_Inputs[self.index])
...
class ONode(Node):
    ...
    def move(self, x, y):
        ...
        # Show inputs of all objects (excluding the already connected object)
        self.Container_Canvas.Show_Inputs(self.item, self.item.Type_Outputs[self.index])
...
class LineShape(Shape):
    ...
    def HitTest(self, x, y):
        ...
        top = (x - self._x[0]) * (self._x[1] - self._x[0]) + (y - self._y[0]) * (
            self._y[1] - self._y[0]
        )
...
class t_BaseShape(Shape):
    ...
    def HitTest(self, x, y):
        # print 'hittest'
        if (x < self._x[0]) or (x > self._x[1]) or (y < self._y[0]) or (y > self._y[1]):
            return False
        else:
            return True
...
class t_RubberBand_Shape(Shape):
    ...
    def HitTest(self, x, y):
        ...
        if (2 * x < x0 - dx) or (2 * x > x0 + dx) or (2 * y < y0 - dy) or (2 * y > y0 + dy):
            return False
        else:
            return True
    ...
    def draw(self, dc):
        ...
        for i in range(1, self.Parent.N_Inputs):
            ...
            if self.Parent.Inputs[i][2]:
                # if PG.OGL_Orientation_Hor :
                if self.Container_Canvas.Orientation_Hor:
                    dc.DrawLine(x - IO_r, y - IO_r, x - IO_r - 7, y - IO_r)
                else:
                    dc.DrawLine(x - IO_r, y - IO_r, x - IO_r, y - IO_r - 7)
...
class DB_Table_Shape(Shape):  # t_BaseShape ):
    ...
    def draw(self, dc):
        ...
        dc.DrawRectangle(int(self._x[0]), int(self._y[0]), int(self._x[1] - self._x[0]), int(h))
