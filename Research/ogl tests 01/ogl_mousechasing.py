...
class OGLCanvas(ogl.ShapeCanvas):
    ...
    def OnMotion(self, event):
        if (
            self.working
        ):  # wx.Yield() re-entry protection.  Don't need this if use wx.SafeYield()  # http://wxpython-users.1045709.n5.nabble.com/wx-Yield-wx-SafeYield-wx-App-Yield-td2371477.html
            event.Skip()
            return
        self.working = False
        ...
        shape.Move(
            dc, event.GetPosition()[0], event.GetPosition()[1], display=True
        ...
        time.sleep(
            0.2
        )  # introduce a slight lag here (or even before the refresh) and a slight smearing will occur
