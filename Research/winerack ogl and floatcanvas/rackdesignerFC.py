...
class ShapeMixin:
    ...
    def Save(self, xmlDoc):
        if isinstance(self, FC.Rectangle):
            ...
            addValueElement(xmlDoc, klass, "width", ucode(self.WH[0]))
            addValueElement(xmlDoc, klass, "height", ucode(self.WH[1]))
            addValueElement(xmlDoc, klass, "x", ucode(self.XY[0]))
            addValueElement(xmlDoc, klass, "y", ucode(self.XY[1]))
        ...
        addValueElement(xmlDoc, klass, "rackitemid", ucode(self.RackItemId))
...
class DrawFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title, position, size):
        ...
        self.NavCanvas = NavCanvas.NavCanvas(
            self, -1, (500, 500), ProjectionFun=None, Debug=0, BackgroundColor="DARK SLATE BLUE"
        )
    ...
    def OnTBAddTri1(self, event):
        rackitemid, shapetip = self.GetRackItem()
        if rackitemid != 0:
            Points = N.array(((0, 100), (100, 0), (0, 0)), N.float_)
...
app = wx.PySimpleApp()
DrawFrame(
    None, -1, "A start for Wine Rack designer using FloatCanvas", wx.DefaultPosition, (700, 700)
)
app.MainLoop()
