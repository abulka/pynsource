...
class DrawFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Canvas = NavCanvas.NavCanvas(
            self, -1, (500, 500), ProjectionFun=None, Debug=0, BackgroundColor="White"
        ).Canvas
    ...
    def AddTree(self, root):
        ...
        def CreateObject(node):
            if node.Children:
                object = NodeObject(
                    node.Name, node.Point, (15, 4), BackgroundColor="Yellow", TextColor="Black"
                )
            else:
                object = MovingTextBox(
                    node.Name,
                    node.Point,
                    2.0,
        ...
        def AddConnectors(node):
            for child in node.Children:
                Connector = ConnectorLine(
                    node.DrawObject, child.DrawObject, LineWidth=3, LineColor="Red"
                )
