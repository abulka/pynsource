...
class MyCanvas(wx.ScrolledWindow):
    def __init__(self, parent, id=-1, size=wx.DefaultSize):
        ...
        self.maxWidth, self.maxHeight = (
            max(self.maxWidth, bmp.GetSize()[0]),
            max(self.maxHeight, bmp.GetSize()[1]),
        )
    ...
    def zoom(self, out=True, reset=False):
        ...
        if TRIMTO_VIRTUALSIZE:
            if BUFFERED:
                self.SetVirtualSize(
                    (self.maxWidth * self.bufferedzoom, self.maxHeight * self.bufferedzoom)
                )
                self.SetScrollbars(1, 1, self.GetVirtualSize()[0], self.GetVirtualSize()[1])
            else:
                if out:
                ...
                self.SetScrollbars(
                    1, 1, self.GetVirtualSize()[0], self.GetVirtualSize()[1], jumptox, jumptoy, True
                )  # boolean is skip refresh or not, doesn't seem to matter

                # If zoom out so much you need to clear the whole window to get rid of bigger image remnants
                if self.NeedToClear():
                    self.clear_whole_window = True
                    self.SetScrollbars(
                        1, 1, self.GetVirtualSize()[0], self.GetVirtualSize()[1], 0, 0, True
    ...
    def CalcVirtSize(self):
        ...
        return (
            self.maxWidth * self.andyscale,
            self.maxHeight * self.andyscale,
        )  # if BUFFERED may need to use self.bufferedzoom instead.
    ...
    def DebugSizez(self, fromwheremsg):
        ...
        print(
            msg
            + "(%s) NeedToClear %s GetVirtualSize %d getWidth %d GetClientSize %d self.GetViewStart() %d self.maxWidth %d "
            % (
                fromwheremsg,
                self.NeedToClear(),
    ...
    def ConvertEventCoords(self, event):  # PEN DRAWING
        # ANDY
        if BUFFERED:
            newpos = self.CalcUnscrolledPosition(
                event.GetX() * self.GetScaleX(), event.GetY() * self.GetScaleY()
            )
        else:
            newpos = self.CalcUnscrolledPosition(event.GetX(), event.GetY())
            newpos = (
                newpos[0] * self.GetScaleX() / self.andyscale,
                newpos[1] * self.GetScaleY() / self.andyscale,
            )
        # print newpos
        return newpos
