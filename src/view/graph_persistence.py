...
class GraphPersistence:
    ...
    def prepare(self, filedata_str):
        ...
        if len(version_data_line) >= len("# PynSource Version n.n") and version_data_line[0] == '#':
            ...
            if version_info[2] != 'Version': return False # unexpected stuff in file
    ...
    def PersistenceConvert(self, to_vers):
        if to_vers == 1.0:
            ...
            self.filedata_list.insert(1, "{'type':'meta', 'info1':'Lorem ipsum dolor sit amet, consectetur adipiscing elit.'}")
            import re
    ...
    def Load(self, filedata_str, force=False):
        ...
        for data in self.filedata_list:
            ...
            if data['type'] == 'umlshape':
                node = self.graph.NotifyCreateNewNode(data['id'], data['x'], data['y'], data['width'], data['height'])
                ...
                self.graph.NotifyOfEdgeCreateFromPersistence(edge, data)  # e.g. UmlGraph class would add edge['uml_edge_type'] if it exists in data
    ...
    def Save(self):
        ...
        for edge in self.graph.edges:
            ...
            subclass_persistence_str = self.graph.NotifyOfEdgeBeingPersisted(edge)
