...
try:
    ...
    def eventStrToList(eventstr):
        ...
        commapositions.append(
            pos
        )  # Add a fake comma position representing the r.h. end of the list
...
def DeriveScenarioFolderPath():
    ...
    currentmoduledir = os.path.dirname(
        os.path.abspath(sys.argv[0])
    )  # better to use os.path.abspath(".")

    if os.path.isdir(
        os.path.join(currentmoduledir, "Scenarios")
    ):  # you might be in .../Devel already!
...
def FilenameToFullScenarioPathName(filename):
    ...
    if not parent:
        ...
        ), "Should not be specifying an extension when supplying a simple filename without a path"
...
if sys.version_info[:2] <= (2, 2):
    ...
    def asciiDate(datetuple, timepos, stryearoverride):
        ...
        resultdate = dateobject + delta
        strdate = (
            resultdate.strftime("%a %b %d ") + stryearoverride + resultdate.strftime(" %H:%M")
        )  # e.g. "Sun Jul 01 1944 06:01"
        # print strdate
        return strdate

    def TimePosToDateStr1943(
        timepos,
        modernyear=2001,
        waryear=int(DEFAULT_WARDATE_USFORMAT[2]),
        month=int(DEFAULT_WARDATE_USFORMAT[0]),
        day=int(DEFAULT_WARDATE_USFORMAT[1]),
        hour=6,
    ):
...
class TestCase00(unittest.TestCase):
    ...
    def checkFullPathForSingleFile(self):
        ...
        try:
            ...
            assert (
                0
            ), "Should not be able to pass in a single filename with an extension cos the import never allowed it so neither should we"
        except:
            pass
