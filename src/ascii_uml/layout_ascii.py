...
class NodeWidthCalc:
    ...
    def calc(self):
        ...
        ):  # TODO need to have every node figure out its own width !!!
...
class model_to_ascii_builder:
    ...
    def CalcRelations(self, node, graph):
        ...
        rels_generalisation = self.removeDuplicates(
            [
                edge["target"].id
                for edge in graph.edges
        ...
        for i in range(len(nodes)):
            ...
            if s:
                ...
                if annotation == "root":
                    ...
                    if root_counter <= 0 or self.next_node_is_fc(nodes, i):
                        w.Flush()
                        row_top_padding = self.cr(
                            row_margin - PADDING_ABOVE_ALIASES
                        )  # "\n\n"  # ONE, TWO
                ...
                # Any fc (first child) needs to be put on a fresh line, but no header margin cos want to glue to parent
                    ...
                    root_counter = (
                        0
                    )  # could potentially set to NUM_LONER_ROOTS_PER_LINE - 1 to keep more things on the same row but fc, tab, root sequence is tricky
            ...
            if rels_generalisation:
                if annotation == "tab":
                    ...
                    s += (
                        "".join(self.list_parents(rels_generalisation)).center(maxwidth, " ") + "\n"
                    )

                if (
                    annotation == "root"
                ):  # and len(rels_generalisation) > 1:   # List multiple parents as aliases when multiple inheritance
                    s += (
                        "".join(self.list_parents(rels_generalisation)).center(maxwidth, " ") + "\n"
                    )
            ...
            def height_of(node):
                ...
                if len(node.meths) and len(
                    node.attrs
                ):  # account for the extra dividing line if have both attrs and methods
                    result += 1
                return result
