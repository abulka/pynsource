...
class CmdDumpUmlWorkspace(CmdBase):
    def execute(self):
        ...
        print "v" * 50
        self.context.model.Dump()
        print
        print "  MISC Scaling etc info "
        print
        self.context.coordmapper.DumpCalibrationInfo(dump_nodes=False)
        print "line-line intersections", len(self.context.model.graph.CountLineOverLineIntersections())
        print "node-node overlaps", self.context.overlap_remover.CountOverlaps()
        print "line-node crossings", self.context.model.graph.CountLineOverNodeCrossings()['ALL']/2 #, self.graph.CountLineOverNodeCrossings()
        print "bounds", self.context.model.graph.GetBounds()
