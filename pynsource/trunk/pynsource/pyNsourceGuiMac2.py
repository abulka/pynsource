...
##            self.associations_composition.remove(shape)
    ...
    def ArrangeShapes2(self):
        shapeslist = self.SortShapes()

        import networkx as NX
        import networkx.drawing.layout as NX2
        ...
        for classShape in shapeslist:
            node = classShape.region1.GetText()
            width, height = self.getShapeSize(classShape)
            sizes55[node] = (width, height)
            G.add_node(node)
            nodeToShapes[node] = classShape
            #G.add_edge("hello", "there")  # todo work out what is connected to what
        print G.nodes()
        print G.edges()

        print "sizes55", sizes55

        #print dir(NX2)
        #print NX2.circular_layout(G)
        res55 = NX2.spring_layout(G)
        print 'res55', res55

        import layoutuml

        coords55 = layoutuml.ReMap(res55, sizes55)
        print
        print 'coords55', coords55

        dc = wxClientDC(self)
        self.PrepareDC(dc)

        self.setSize(wxSize(int(2000), int(2000))) # fudge guess
        for (node,xy) in coords55.iteritems():
            # find the shape corresponding to node
            classShape = nodeToShapes[node]
            x, y = xy
            classShape.Move(dc, x, y, false)
            print 'Moving', node, 'to', x, y
